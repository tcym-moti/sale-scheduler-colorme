...
- 固定セール価格または割引率の指定
- JST表示、UTC保存の開始・終了日時
- 実行前Previewと重複予約チェック（対象数、開始・終了の概算処理時間を表示）
- ColorMe API制限を守り、指定日時から商品ごとに順次反映
...
- 料金はコードへ固定していません。
- 複数商品は指定日時から順次処理します。全商品が同じ秒に反映されることは保証しません。API retryや通信状況により概算時間を超える場合があります。
...
公開前に、運営者名と問い合わせ先をproduction環境変数へ設定してください。未設定時は公開ページに設定待ちの表示が残ります。
